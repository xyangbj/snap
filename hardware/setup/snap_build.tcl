...
if { $write_user_dcp == "TRUE" } {
  ...
  puts [format "%-*s %-*s %-*s %-*s"  $widthCol1 "" $widthCol2 "start OOC synthesis" $widthCol3 "with directive: $directive" $widthCol4 "[clock format [clock seconds] -format %H:%M:%S]"]
  if { [catch "$command > $logfile" errMsg] } {
    puts [format "%-*s %-*s %-*s %-*s"  $widthCol1 "" $widthCol2 "" $widthCol3 "ERROR: OOC synthesis failed" $widthCol4 "" ]
