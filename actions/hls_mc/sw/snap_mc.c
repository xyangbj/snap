...
static void snap_prepare_mc(struct snap_job *cjob,
                                 struct mc_job_t *mjob,
                                 void *addr_in,
                                 uint32_t size_in,
                                 uint16_t type_in,
                                 void *addr_out,
...
int main(int argc, char *argv[])
{
    ...
    action = snap_attach_action(card, MC_ACTION_TYPE, action_irq, 60);
    if (action == NULL) {
    ...
    snap_prepare_mc(&cjob, &mjob,
                         (void *)addr_in,  0, type_in,
    ...
    pFILE_out = fopen(output, "w");
    if(pFILE_out == NULL)
    {
       printf("open output file fail!!!\n");     
    }
    printf("result %f\n", mjob.result);
    fprintf(pFILE_out, "result %f\n", mjob.result);
    fclose(pFILE_out);
